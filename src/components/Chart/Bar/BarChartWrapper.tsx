 ...
 * @constructor
 */
function BarChartWrapper({title, description, dataset, options, keys, stackMode = 'normal', yFormat = 'number'}: IBarChart) {
    const chartType = 'bar';
    ...
    useEffect((): void => {
        if (!dataset) {
            ...
            if(isDatasetEmpty(dataset)) {
                ...
                let dataRecalc = stackMode === 'percentage100' ? calculateDataPercentage(dataset, keys) : dataset;
