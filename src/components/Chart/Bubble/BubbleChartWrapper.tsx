...
import {
  ...
  createBubbleChartTooltipDataset,
...
} from "../utils/data/dataInterfaces";
import {
  checkResponseError,
  extractError,
  isDatasetEmpty,
...
function BubbleChartWrapper({
  ...
  const [canvasSettings, setCanvasSettings]: [
    IChartJSDatasets | undefined,
    any,
  ] = useState(undefined);
  const [chartData, setData]: [IChartJSDatasets, any] =
    useState<IChartJSDatasets>({ labels: [], datasets: [] });
  const [tooltipData, setTooltipData]: [ICustomTooltipDataset[] | null, any] =
    useState(null);
