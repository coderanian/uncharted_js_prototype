 ...
 * @constructor
 */
function AreaChartWrapper({title, description, dataset, options, keys, stackMode = 'normal', yFormat = 'number'}: IAreaChart) {
    ...
    useEffect((): void => {
        if (!dataset) {
            ...
            if (isDatasetEmpty(dataset)) {
                ...
                let dataRecalc: IDataset<string | number>[] = stackMode === 'percentage100' ? calculateDataPercentage(dataset, keys) : dataset;
