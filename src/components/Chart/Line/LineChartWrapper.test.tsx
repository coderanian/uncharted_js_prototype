...
describe("<AreaChartWrapper />", () => {
  ...
  it("prepares correct chart dataset by extracting correct labels, data categories and colors", () => {
    ...
    expect(
      chartDataSetupFactory(testData, "bar", ["a", "b", "c"]),
    ).toStrictEqual({
      datasets: [
        {
  ...
  it("sets correct canvas settings", () => {
    ...
    // @ts-ignore
    expect(canvasSettings.scales.x.title.display).toBe(
      expectedResult.scales.x.title.display,
    );
    // @ts-ignore
    expect(canvasSettings.scales.x.grid.display).toBe(
      expectedResult.scales.x.grid.display,
    );
    // @ts-ignore
    expect(canvasSettings.scales.y.title.display).toBe(
      expectedResult.scales.y.title.display,
    );
    // @ts-ignore
    expect(canvasSettings.scales.y.stacked).toBe(
      expectedResult.scales.y.stacked,
    );
    // @ts-ignore
    expect(canvasSettings.scales.y.grid.display).toBe(
      expectedResult.scales.y.grid.display,
    );
    // @ts-ignore
  ...
  it("creates correct tick format", () => {
    ...
    let testData = [{ aggregationKey: "1", a: 50, b: 20, c: 30 }];
    expect(
      formatValue(testData[0].aggregationKey, options.xFormat),
    ).toStrictEqual("1");
    expect(formatValue(testData[0].b, options.yLeftFormat)).toStrictEqual(
      "20%",
    );
    expect(formatValue(testData[0].c, options.yRightFormat)).toStrictEqual(
      "30€",
    );
  });
});
